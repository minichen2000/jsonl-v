...
- **行列表**：每行显示行号、摘要（取 `type`/`role` 字段）、字节数（KB 标橙，>100KB 标红）。
  单击选中（列表不跳动），右键可复制原始行/格式化文本/纯文本查看。
- **树视图**：JSON 可折叠树，键/字符串/数字/布尔着色；Tab 栏右侧有「全展开 / 全折叠」。
- **长字符串**：值太长或含换行时，旁边出现「📄纯文本查看」按钮——弹窗按**真实换行**显示
  解转义后的文本（等宽字体、可换行、一键复制、可全屏、可编辑不保存、右键拷贝选中）。
  这是看 systemPrompt、思考内容、工具输出的正确姿势。
- **格式化文本 / 原始行** 页签：格式化后的 JSON / 未解析的一行原文。
...
- **完整上下文（还原）**：选中一个 llm.request 行，详情切到「完整上下文（还原）」页签——
  ...
  每条目附来源徽标 `〔role · 侧〕`，按「🖥 宿主/用户侧 / 🤖 LLM 输出」分段，
  顶部有双侧条目数与字节统计；工具条目显示完整调用 id，result 标注配对工具名
- **事件过滤**：顶栏下拉框只显示某类事件
...
内嵌 [Noto Emoji](https://github.com/googlefonts/noto-emoji) 字体（SIL OFL 1.1，见 `assets/NotoEmoji-OFL.txt`）用于 emoji 显示。
